...
function initGame() {
    ...
    // ثبت سرویس ورکر برای قابل نصب شدن بازی
    ...
    if (eventCloseBtn) {
        eventCloseBtn.onclick = () => document.getElementById('event-panel').classList.replace('event-panel-visible', 'event-panel-hidden');
    }
    ...
    style.innerHTML = `
        ...
        @media (max-width: 768px) {
            ...
            .top-bar-center { 
                ...
                border-top: 1px solid rgba(244, 208, 63, 0.3) !important; 
                justify-content: center !important;
                flex-wrap: wrap !important;
            }
    ...
    const angryBtn = document.getElementById('angryMoveBtn');
    if (angryBtn) angryBtn.onclick = () => angryMoveModal.style.display = 'none';
    ...
    complaintModal.innerHTML = `
        <div style="flex: 1; text-align: right;">
            <h3 style="color: #e74c3c; margin-bottom: 10px; font-size: 1rem;">گزارش شهروندان</h3>
            <p id="complaintText" style="color: #e8dcc8; font-size: 0.85rem; line-height: 1.6;">ببین اقام رییس، ما ۵ نفری توی یک خونه زندگی می‌کنیم، دیوونه شدیم! در حالی که خونه ی دیگه ات جمعیتش کمتره! یه کاری بکن لطفاً...</p>
    ...
    if (complaintBtn) complaintBtn.onclick = () => {
    ...
    const starvationBtn = document.getElementById('starvationBtn');
    ...
    const txtCancelUnlock = document.getElementById('txtCancelUnlock');
    if (txtCancelUnlock) txtCancelUnlock.onclick = () => unlockModal.style.display = 'none';
    ...
    const txtCancelDispatch = document.getElementById('txtCancelDispatch');
    if (txtCancelDispatch) txtCancelDispatch.onclick = () => {
    ...
    if (closeRequestsPanel) closeRequestsPanel.onclick = () => panelRequests.classList.remove('panel-open');

    const panelBuild = document.getElementById('panelBuild');
    if (panelBuild) panelBuild.innerHTML = `
        <div class="panel-header" style="display:flex; justify-content:space-between; align-items:center; padding:12px 16px; border-bottom:1px solid rgba(201,168,76,0.1);">
    ...
    const txtCloseResult = document.getElementById('txtCloseResult');
    if (txtCloseResult) txtCloseResult.onclick = () => resultModal.style.display = 'none';
    ...
    if (txtResume) txtResume.onclick = togglePauseMenu;
    const txtExit = document.getElementById('txtExit');
    if (txtExit) txtExit.onclick = () => {
        saveGame(false, true);
    };
    
    const txtSettings1 = document.getElementById('txtSettings1');
    if (txtSettings1) txtSettings1.onclick = () => {
        document.getElementById('pauseModal').style.display = 'none';
        document.getElementById('settingsModal').style.display = 'flex';
    };
    const txtBack = document.getElementById('txtBack');
    if (txtBack) txtBack.onclick = () => {
        document.getElementById('settingsModal').style.display = 'none';
        document.getElementById('pauseModal').style.display = 'flex';
    };
    ...
    if (tabControls) tabControls.onclick = () => switchTab('controls');
    ...
    if (btnMute) btnMute.onclick = () => {
        if (bgMusic.volume > 0) {
            ...
            btnMute.innerText = LANG[gameState.currentLang].unmute;
        } else {
            bgMusic.volume = 0.4;
            if (volumeSlider) volumeSlider.value = 0.4;
            btnMute.innerText = LANG[gameState.currentLang].mute;
        }
    };
    ...
    updateBindTexts();
    const bindUp = document.getElementById('bindUp');
    if (bindUp) bindUp.onclick = () => { gameState.rebindingKey = 'up'; bindUp.innerText = '...'; };
    const bindDown = document.getElementById('bindDown');
    if (bindDown) bindDown.onclick = () => { gameState.rebindingKey = 'down'; bindDown.innerText = '...'; };
    const bindLeft = document.getElementById('bindLeft');
    if (bindLeft) bindLeft.onclick = () => { gameState.rebindingKey = 'left'; bindLeft.innerText = '...'; };
    const bindRight = document.getElementById('bindRight');
    if (bindRight) bindRight.onclick = () => { gameState.rebindingKey = 'right'; bindRight.innerText = '...'; };
    ...
    if (btnBuild) btnBuild.onclick = () => {
        ...
        if (panelBuild) panelBuild.classList.add('panel-open');
    ...
    const closeBuildPanel = document.getElementById('closeBuildPanel');
    if (closeBuildPanel) closeBuildPanel.onclick = () => panelBuild && panelBuild.classList.remove('panel-open');
    ...
    if (txtBuildBtn) txtBuildBtn.onclick = () => {
        ...
        if (panelBuild) panelBuild.classList.remove('panel-open');
    ...
    if (exploreBtn) exploreBtn.onclick = () => {
        ...
        if (panelExplore) panelExplore.classList.add('panel-open');
        if (gameState.tutorialStep === 5) { gameState.tutorialStep = 6; updateTutorialBox(); }
    };
    const closeExplorePanel = document.getElementById('closeExplorePanel');
    if (closeExplorePanel) closeExplorePanel.onclick = () => panelExplore && panelExplore.classList.remove('panel-open');
    ...
    if (requestsBtn) requestsBtn.onclick = () => {
        ...
        if (panelRequests) panelRequests.classList.add('panel-open');
    };
    ...
    const dispatchLut = document.getElementById('dispatchLut');
    if (dispatchLut) dispatchLut.onclick = () => openDispatchModal('کویر لوت');
    const dispatchLaton = document.getElementById('dispatchLaton');
    if (dispatchLaton) dispatchLaton.onclick = () => openDispatchModal('آبشار لاتون');
    const dispatchPersepolis = document.getElementById('dispatchPersepolis');
    if (dispatchPersepolis) dispatchPersepolis.onclick = () => openDispatchModal('تخت جمشید');

    const closeMovePopPanel = document.getElementById('closeMovePopPanel');
    if (closeMovePopPanel) closeMovePopPanel.onclick = () => {
        if (panelMovePop) panelMovePop.classList.remove('panel-open');
...
window.onload = initGame;    let modal = document.getElementById('saveNameModal');
    if (!modal) {
        modal = document.createElement('div');
        modal.id = 'saveNameModal';
        modal.style.cssText = "position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.95); z-index: 7000; display: none; justify-content: center; align-items: center; font-family: 'Vazirmatn', sans-serif;";
...
function initGame() {
    ...
    if (heroActions && startBtnHero) {
        ...
        installBtn.className = 'btn-primary';
        installBtn.innerText = 'نصب بازی';
        installBtn.style.display = 'none'; // در ابتدا مخفی است
        installBtn.style.marginRight = '15px';
        installBtn.style.background = 'linear-gradient(135deg, #8e44ad, #9b59b6)'; 
        heroActions.insertBefore(installBtn, startBtnHero);

        window.addEventListener('beforeinstallprompt', (e) => {
            e.preventDefault();
            deferredPrompt = e;
            installBtn.style.display = 'inline-block'; // وقتی مرورگر اجازه نصب داد، دکمه ظاهر می‌شود
        });
        ...
        installBtn.addEventListener('click', async () => {
            if (deferredPrompt) {
                ...
                showNotification("برای نصب، از منوی مرورگر خود گزینه Add to Home Screen را انتخاب کنید.", "info");
            }
        });
    }
