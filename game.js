...
let bgMusic = null; 
let deferredPrompt = null; // متغیر برای ذخیره رویداد نصب PWA
...
function getAllSaves() {
    try { return JSON.parse(localStorage.getItem('lastWarmthSaves')) || {}; } 
    catch (e) { return {}; }
}
    ...
    if (!modal) {
        ...
        document.getElementById('confirmSaveNameBtn').onclick = () => {
            ...
            if (name) {
                ...
                if (saved && !exitAfter) showNotification("بازی با موفقیت ذخیره شد!", "success");
            } else { showNotification("لطفاً یک نام معتبر وارد کنید.", "warning"); }
        };
...
function loadGame(saveName) {
    try {
        ...
        if (savedData) {
            gameState.currentSaveName = saveName; gameState.day = savedData.day; gameState.population = savedData.population; gameState.fuel = savedData.fuel; gameState.food = savedData.food; gameState.wood = savedData.wood; gameState.stone = savedData.stone; gameState.heat = savedData.heat; gameState.hope = savedData.hope; gameState.satisfaction = savedData.satisfaction;
            gameState.HOUSE_HEXES = savedData.HOUSE_HEXES; gameState.POWERPLANT_HEXES = savedData.POWERPLANT_HEXES; gameState.unlockedHexes = savedData.unlockedHexes; gameState.tutorialStep = savedData.tutorialStep !== undefined ? savedData.tutorialStep : 0;
            return true;
        }
    } catch (e) {}
    return false;
...
function deleteGame(saveName) {
    let saves = getAllSaves();
    if (saves[saveName]) { delete saves[saveName]; localStorage.setItem('lastWarmthSaves', JSON.stringify(saves)); }
}

function showGameOver() {
    gameState.gameOver = true;
    if (gameState.currentSaveName) { deleteGame(gameState.currentSaveName); gameState.currentSaveName = null; }
    ...
    if (!goScreen) {
        ...
        if (btnGoMenu) btnGoMenu.onclick = () => window.location.reload();
...
function continueGame() {
    ...
    if (closeLoadModal) closeLoadModal.onclick = () => { modal.style.display = 'none'; };
    document.querySelectorAll('.load-save-btn').forEach(btn => {
        btn.onclick = () => {
            let name = btn.getAttribute('data-name');
            if (loadGame(name)) { modal.style.display = 'none'; showStoryScreen(); startActualGame(); }
        };
    });
    document.querySelectorAll('.delete-save-btn').forEach(btn => {
        btn.onclick = () => {
            ...
            if (confirm("آیا از حذف این بازی مطمئن هستید؟")) {
                deleteGame(name);
                if (!hasSavedGame()) { modal.style.display = 'none'; } else { continueGame(); }
            }
        };
    });
}
...
function drawMap() {
    ...
    gameState.hexes.forEach(hex => {
        ...
        let isHouse = !!houseData, isPP = !!ppData;
        ...
        if (isHouse && houseImg.complete && houseImg.naturalHeight !== 0) {
            ...
            const pillW = textWidth + 30, pillH = 22, pillX = hex.x + 15, pillY = hex.y - 35;
            ...
            ctx.font = "12px Arial"; ctx.fillStyle = '#f4d03f'; ctx.textAlign = 'left'; ctx.textBaseline = 'middle';
            ctx.fillText('👤', pillX + 6, pillY + pillH/2 + 1);
            
            ctx.font = "bold 13px 'Vazirmatn', sans-serif"; ctx.fillStyle = '#fff'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
        ...
        if (site) {
            ...
            ctx.beginPath(); ctx.arc(hex.x, hex.y, 20, -Math.PI / 2, (-Math.PI / 2) + (Math.PI * 2 * progress));
            ctx.lineWidth = 4; ctx.strokeStyle = '#f4d03f'; ctx.stroke();
            ctx.font = "bold 14px 'Vazirmatn', sans-serif"; ctx.fillStyle = '#fff'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
...
function setupControls() {
    ...
    window.addEventListener('keydown', e => {
        if (e.code === 'Escape') {
            ...
            if (gs && gs.style.display === 'block') {
                ...
                if (sm && sm.style.display === 'flex') { sm.style.display = 'none'; const pm = document.getElementById('pauseModal'); if(pm) pm.style.display = 'flex'; } 
        ...
        if (gameState.rebindingKey) {
            ...
            gameState.rebindingKey = null; return;
        }
    ...
    canvas.addEventListener('contextmenu', e => {
        e.preventDefault();
        if (gameState.isPlacing) { gameState.isPlacing = false; showNotification(LANG[gameState.currentLang].buildCancel, "info"); }
        if (gameState.isMovingPop) { gameState.isMovingPop = false; showNotification(LANG[gameState.currentLang].moveCancel, "info"); }
    });
    ...
    let pinchInitialDistance = 0, pinchInitialZoom = 1;
...
function togglePauseMenu() {
    ...
    if(pm) pm.style.display = gameState.isPaused ? 'flex' : 'none';
    if (!gameState.isPaused) { const sm = document.getElementById('settingsModal'); if(sm) sm.style.display = 'none'; }
}

function updateBindTexts() {
    const elUp = document.getElementById('bindUp'); if(elUp) elUp.innerText = gameState.keyBindings.up.replace('Key', '');
    const elDown = document.getElementById('bindDown'); if(elDown) elDown.innerText = gameState.keyBindings.down.replace('Key', '');
    const elLeft = document.getElementById('bindLeft'); if(elLeft) elLeft.innerText = gameState.keyBindings.left.replace('Key', '');
    const elRight = document.getElementById('bindRight'); if(elRight) elRight.innerText = gameState.keyBindings.right.replace('Key', '');
}
...
function handleMapClick() {
    ...
    if (gameState.isPlacingMigrants) {
        ...
        if (destHouse && destHouse.pop < 5) {
            ...
            if (gameState.migrantsToPlace === 0) {
                ...
                for (let h of gameState.migrantTargetHouses) { if (h.pop > 3) { causedOverpopulation = true; break; } }
                if (causedOverpopulation) { gameState.satisfaction = Math.max(0, gameState.satisfaction - 1); showNotification("خونه‌های پذیرنده شلوغ شد! ۱ درصد رضایت کم شد.", "warning"); } 
                else { gameState.satisfaction = Math.min(100, gameState.satisfaction + 1); showNotification("میزبانی عالی بود! ۱ درصد رضایت بیشتر شد.", "success"); }
                updateUI();
            }
        } else { showNotification("ظرفیت این خانه پر است!", "warning"); }
        return;
    }
    ...
    if (gameState.isMovingPop) {
        ...
        if (destHouse && sourceHouse && destHouse !== sourceHouse) {
            if (destHouse.pop + gameState.moveAmount > 5) { showNotification("ظرفیت این خانه پر است! حداکثر ۵ نفر در یک خانه جا می‌گیرند.", "warning"); gameState.isMovingPop = false; return; }
            ...
            if (sourceHouse.receivedToday) { showAngryMoveModal(); sourceHouse.receivedToday = false; gameState.isMovingPop = false; return; }
            sourceHouse.pop -= gameState.moveAmount; destHouse.pop += gameState.moveAmount; destHouse.receivedToday = true; 
            updateUI(); gameState.isMovingPop = false;
            if (gameState.tutorialStep === 16) { gameState.tutorialStep = 17; updateTutorialBox(); }
        } else { showNotification(LANG[gameState.currentLang].selectOther, "warning"); }
        return;
    }
    ...
    if (isHouse && !gameState.isPlacing) {
        ...
        if (maxTransferable > 0) {
            if (gameState.tutorialStep === 14 || gameState.tutorialStep === 0) {
                gameState.moveSource = target; openMovePopPanel(maxTransferable);
                if (gameState.tutorialStep === 14) { gameState.tutorialStep = 15; updateTutorialBox(); }
            } else if (gameState.tutorialStep > 0) { showNotification("لطفاً طبق آموزش پیش برو!", "warning"); }
        } else { showNotification("آدمی برای انتقال در این خانه نیست!", "info"); }
        return;
    }
    ...
    if (isLocked && !isUnlocked && !isHouse && !isPP && !isOccupied) {
        ...
        const um = document.getElementById('unlockModal'); if(um) um.style.display = 'block';
        return;
    }
        ...
        if (gameState.placingType === 'house') {
            if (gameState.tutorialStep === 4 || gameState.tutorialStep === 0) {
                if (gameState.wood >= 10) {
                    gameState.wood -= 10; updateUI(); gameState.isPlacing = false;
                    gameState.constructionSites.push({ q: target.q, r: target.r, endTime: Date.now() + 30000, completed: false, type: 'house' });
                    if (gameState.tutorialStep === 4) { gameState.tutorialStep = 5; updateTutorialBox(); }
                    showNotification(LANG[gameState.currentLang].buildStart, "success");
                } else { showNotification(LANG[gameState.currentLang].noWood, "warning"); }
            } else if (gameState.tutorialStep > 0) { showNotification("فعلاً طبق آموزش پیش برو!", "warning"); }
        } else if (gameState.placingType === 'powerplant') {
            if (gameState.tutorialStep === 12 || gameState.tutorialStep === 0) { 
                if (gameState.stone >= 10) {
                    gameState.stone -= 10; updateUI(); gameState.isPlacing = false;
                    gameState.constructionSites.push({ q: target.q, r: target.r, endTime: Date.now() + 30000, completed: false, type: 'powerplant' });
                    if (gameState.tutorialStep === 12) { gameState.tutorialStep = 13; updateTutorialBox(); }
...
function updateTutorialBox() {
    ...
    if (gameState.tutorialStep === 0) { box.style.display = 'none'; btn.style.display = 'none'; pointer.style.display = 'none'; return; }
    ...
    if (gameState.tutorialStep === -1) {
        ...
        btn.style.display = 'inline-block'; btn.innerText = "متوجه شدم"; pointer.style.display = 'none';
        ...
        btn.style.display = 'inline-block'; btn.innerText = "پایان آموزش"; pointer.style.display = 'none';
...
function movePointer(elementId) {
    ...
    } else if (pointer) { pointer.style.display = 'none'; }
}
...
function startExpedition(regionName, troops) {
    ...
    if (mainHouse) mainHouse.pop = Math.max(0, mainHouse.pop - troops);
    updateUI();
...
function calculateExpeditionResult(exp) {
    if (exp.isTutorial) return { wood: 10, stone: 10, food: 0, fuel: 0, casualties: 0 };
    const regionData = {
    ...
    if (Math.random() < (exact - casualties)) casualties++;
    casualties = Math.min(casualties, exp.troops);
    const survivors = exp.troops - casualties;
    return { wood: survivors * r.res.wood, stone: survivors * r.res.stone, food: survivors * r.res.food, fuel: survivors * r.res.fuel, casualties };
}
...
function showMigrantModal(count) {
    ...
    modal.style.display = 'flex';
    const migrantReject = document.getElementById('migrantReject');
    if(migrantReject) migrantReject.onclick = () => {
        modal.style.display = 'none';
        let canFit = calculateCanFit(count);
        if (canFit) { gameState.satisfaction = Math.max(0, gameState.satisfaction - 1); showNotification("شهروندان ناراحت شدند که با وجود داشتن جا، کمک نکردید. ۱ درصد رضایت کم شد.", "warning"); } 
        else { gameState.satisfaction = Math.min(100, gameState.satisfaction + 1); showNotification("شهروندان خوشحال شدند که منابع تقسیم نشد و بقا تضمین شد. ۱ درصد رضایت بیشتر شد.", "info"); }
        updateUI();
    };
    const migrantWait = document.getElementById('migrantWait');
    if(migrantWait) migrantWait.onclick = () => {
        modal.style.display = 'none';
        gameState.migrantRequests.push({ id: Date.now(), count: count, endTime: Date.now() + 180000 });
        showNotification("آدم‌ها در انتظار نگه داشته شدند. به بخش درخواست‌ها مراجعه کنید.", "info");
    };
    const migrantAccept = document.getElementById('migrantAccept');
    if(migrantAccept) migrantAccept.onclick = () => { modal.style.display = 'none'; startPlacingMigrants(count); };
}
...
function nextDay() {
    ...
    if (gameState.food < totalPop) {
        starvedCount = totalPop - gameState.food; gameState.food = 0;
        ...
        if (remainingPop <= 0) { showGameOver(); return; }
        ...
        if (modal) { document.getElementById('starvationText').innerText = `به دلیل کمبود غذا، ${starvedCount} نفر از شهروندان جان باختند. لطفاً منابع غذایی رو مدیریت کن!`; modal.style.display = 'flex'; }
    } else { gameState.food -= totalPop; }
    ...
    if (hasOvercrowded && hasSpace && gameState.HOUSE_HEXES.length > 1) { const cm = document.getElementById('complaintModal'); if(cm) cm.style.display = 'flex'; }
    updateUI();
    if (Math.random() < 0.33) { let numMigrants = Math.floor(Math.random() * gameState.day) + 1; showMigrantModal(numMigrants); }
}
...
function updateUI() {
    ...
    if (totalPop > 0) gameState.heat = Math.min(100, Math.round((heatCapacity / totalPop) * 100)); 
    else gameState.heat = 0;
    ...
    const updateBar = (id, value, max) => {
        ...
        if (valEl) {
            ...
            if (barEl) { let percentage = max > 0 ? Math.min(100, (value / max) * 100) : 0; barEl.style.width = percentage + '%'; }
            if (id === 'heat') {
...
function startActualGame() {
    ...
    applyLang('fa'); updateTutorialBox();
...
function adjustMapTop() {
    ...
    if (topBar && mapContainer) mapContainer.style.top = topBar.offsetHeight + 'px';
}

function closeAllPanels(exceptId) {
    const panels = ['panelBuild', 'panelExplore', 'panelRequests', 'panelMovePop'];
    panels.forEach(id => { if (id !== exceptId) { const p = document.getElementById(id); if (p) p.classList.remove('panel-open'); } });
}
...
function openMovePopPanel(maxPop) {
    ...
    for(let i=1; i<=maxPop; i++) {
        ...
        b.onclick = () => {
            ...
            document.querySelectorAll('#movePopBtns button').forEach(btn => { btn.style.background = '#2c3e50'; btn.style.color = '#fff'; });
            b.style.background = '#f4d03f'; b.style.color = '#000';
        };
        btnsDiv.appendChild(b);
    }
    gameState.moveAmount = 1;
    if(btnsDiv.firstChild) btnsDiv.firstChild.click(); 
...
function initGame() {
    ...
    window.addEventListener('beforeunload', function (e) {
        const gs = document.getElementById('game-screen');
        if (gs && gs.style.display === 'block' && !gameState.gameOver && gameState.currentSaveName) saveGame(true); 
    });

    const startBtnHero = document.getElementById('startBtnHero');
    if (startBtnHero) {
        startBtnHero.onclick = () => {
            gameState.currentSaveName = null; 
            showStoryScreen();
        };
    }

    const startActualGameBtn = document.getElementById('startActualGameBtn');
    if (startActualGameBtn) {
        startActualGameBtn.onclick = startActualGame;
    }
    ...
    if (navCta) {
        ...
        navCta.addEventListener('click', (e) => {
            e.preventDefault();
            if (deferredPrompt) {
                navCta.innerText = 'در حال نصب...';
                deferredPrompt.prompt();
                deferredPrompt.userChoice.then((choiceResult) => {
                    if (choiceResult.outcome === 'accepted') {
                        showNotification("بازی با موفقیت نصب شد!", "success");
                        navCta.innerText = 'بازی نصب شد';
                        navCta.style.opacity = '0.5';
                ...
                if (isIOS) {
                    showNotification("در آیفون: روی دکمه Share بزنید و Add to Home Screen را انتخاب کنید.", "info");
                } else {
                    showNotification("برای نصب، از منوی مرورگر (سه نقطه) گزینه Install App را انتخاب کنید.", "info");
                }
            }
        });
    ...
    if (eventCloseBtn) eventCloseBtn.onclick = () => { const ep = document.getElementById('event-panel'); if(ep) ep.classList.replace('event-panel-visible', 'event-panel-hidden'); };
    ...
    style.innerHTML = `
        ...
        @media (max-width: 768px) {
            ...
            #btnPause { position: absolute !important; top: 0 !important; left: 0 !important; height: 40px !important; width: 40px !important; display: flex !important; align-items: center !important; justify-content: center !important; transform: none !important; }
            #btnPause img { width: 24px !important; height: 24px !important; }
            .top-bar-left { position: absolute !important; top: 0 !important; right: 0 !important; height: 40px !important; padding: 0 10px !important; display: flex !important; align-items: center !important; gap: 8px !important; }
            ...
            .resource-item { padding: 2px 6px !important; height: 26px !important; gap: 2px !important; margin-bottom: 4px !important; display: inline-flex !important; align-items: center !important; justify-content: center !important; white-space: nowrap !important; }
            .resource-icon { font-size: 0.7rem !important; line-height: 1 !important; display: inline-flex !important; align-items: center !important; justify-content: center !important; transform: scale(0.8) !important; transform-origin: center !important; margin: 0 -1px !important; }
            ...
            .hero-actions { flex-wrap: wrap; justify-content: center; gap: 10px !important; }
            .btn-primary { margin-right: 0 !important; width: 100%; }
            
            /* نمایش دکمه نصب بازی در هدر موبایل */
            .nav-cta { 
    ...
    if (gameScreen) gameScreen.appendChild(nextDayBtn);
    
    nextDayBtn.onmouseover = () => { const img = nextDayBtn.querySelector('img'); img.style.filter = 'brightness(1.2) drop-shadow(0 0 10px rgba(244, 208, 63, 0.9))'; img.style.transform = 'scale(1.1)'; };
    nextDayBtn.onmouseout = () => { const img = nextDayBtn.querySelector('img'); img.style.filter = 'none'; img.style.transform = 'scale(1)'; };
    ...
    window.applyNextDayMobileStyle = function() {
        ...
        if (btn) {
            ...
            if (isMobile) {
                ...
                if (img) { img.style.width = MOBILE_NEXT_DAY_BTN_SIZE + 'px'; img.style.height = MOBILE_NEXT_DAY_BTN_SIZE + 'px'; }
            } else {
                btn.style.top = NEXT_DAY_BTN_TOP + 'px';
                if (img) { img.style.width = NEXT_DAY_BTN_SIZE + 'px'; img.style.height = NEXT_DAY_BTN_SIZE + 'px'; }
            }
        }
        adjustMapTop(); 
    };
    ...
    angryMoveModal.innerHTML = `<div style="flex: 1; text-align: right;"><h3 style="color: #e74c3c; margin-bottom: 10px; font-size: 1rem;">گزارش شهروندان</h3><p style="color: #e8dcc8; font-size: 0.85rem; line-height: 1.6;">بابا ما مگه چقدر جون داریم؟ هی از این خونه به اون خونه می‌بریمون... یه کم آروم باش!</p><button id="angryMoveBtn" style="margin-top: 12px; padding: 8px 20px; background: linear-gradient(145deg, #e74c3c, #c0392b); border: none; border-radius: 6px; color: #fff; font-weight: 700; cursor: pointer;">متوجه شدم</button></div><img src="angry_move.png?t=${new Date().getTime()}" style="width: 90px; height: 90px; object-fit: contain; border-radius: 10px; background: #111; border: 1px solid #333; flex-shrink: 0;">`;
    if (gameScreen) gameScreen.appendChild(angryMoveModal);
    const angryBtn = document.getElementById('angryMoveBtn'); if (angryBtn) angryBtn.onclick = () => angryMoveModal.style.display = 'none';
    ...
    complaintModal.innerHTML = `<div style="flex: 1; text-align: right;"><h3 style="color: #e74c3c; margin-bottom: 10px; font-size: 1rem;">گزارش شهروندان</h3><p id="complaintText" style="color: #e8dcc8; font-size: 0.85rem; line-height: 1.6;">ببین اقام رییس، ما ۵ نفری توی یک خونه زندگی می‌کنیم، دیوونه شدیم! در حالی که خونه ی دیگه ات جمعیتش کمتره! یه کاری بکن لطفاً...</p><button id="complaintBtn" style="margin-top: 12px; padding: 8px 20px; background: linear-gradient(145deg, #e74c3c, #c0392b); border: none; border-radius: 6px; color: #fff; font-weight: 700; cursor: pointer;">متوجه شدم</button></div><img src="complaint.png?t=${new Date().getTime()}" style="width: 90px; height: 90px; object-fit: contain; border-radius: 10px; background: #111; border: 1px solid #333; flex-shrink: 0;">`;
    if (gameScreen) gameScreen.appendChild(complaintModal);
    const complaintBtn = document.getElementById('complaintBtn'); if (complaintBtn) complaintBtn.onclick = () => { complaintModal.style.display = 'none'; gameState.satisfaction = Math.max(0, gameState.satisfaction - 1); updateUI(); showNotification("۱ درصد رضایت کم شد!", "warning"); };
    ...
    starvationModal.innerHTML = `<div style="flex: 1; text-align: right;"><h3 style="color: #e74c3c; margin-bottom: 10px; font-size: 1rem;">گزارش تلفات</h3><p id="starvationText" style="color: #e8dcc8; font-size: 0.85rem; line-height: 1.6;"></p><button id="starvationBtn" style="margin-top: 12px; padding: 8px 20px; background: linear-gradient(145deg, #e74c3c, #c0392b); border: none; border-radius: 6px; color: #fff; font-weight: 700; cursor: pointer;">متوجه شدم</button></div><img src="starvation.png?t=${new Date().getTime()}" style="width: 90px; height: 90px; object-fit: contain; border-radius: 10px; background: #111; border: 1px solid #333; flex-shrink: 0;">`;
    if (gameScreen) gameScreen.appendChild(starvationModal);
    const starvationBtn = document.getElementById('starvationBtn'); if (starvationBtn) starvationBtn.onclick = () => starvationModal.style.display = 'none';
    ...
    unlockModal.innerHTML = `<h3 id="txtUnlockTitle" style="color: #f4d03f; margin-bottom: 15px;">باز کردن قفل</h3><p id="txtUnlockQ" style="color: #e8dcc8; margin-bottom: 5px;">آیا می‌خواهید این قفل را باز کنید؟</p><p id="txtUnlockCost" style="color: #aaa; font-size: 0.8rem; margin-bottom: 20px;">این کار ۱۰ سنگ هزینه دارد.</p><div style="display: flex; gap: 10px;"><button id="txtUnlockYes" style="flex:1; padding:10px; background:linear-gradient(145deg, #f4d03f, #c9a84c); border:none; border-radius:6px; color:#1a1a2e; font-weight:700; cursor:pointer;">بله، باز کن</button><button id="txtCancelUnlock" style="flex:1; padding:10px; background:transparent; border:1px solid #8a7a6a; border-radius:6px; color:#f5e6c8; cursor:pointer;">انصراف</button></div>`;
    if (gameScreen) gameScreen.appendChild(unlockModal);
    const txtUnlockYes = document.getElementById('txtUnlockYes');
    if (txtUnlockYes) txtUnlockYes.onclick = () => {
        unlockModal.style.display = 'none';
        if (gameState.stone >= 10) {
            gameState.stone -= 10; gameState.unlockedHexes.push(gameState.pendingUnlockTarget); updateUI();
            showNotification("قفل باز شد و ۱۰ سنگ کم شد!", "success");
            if (gameState.tutorialStep === 1) { gameState.tutorialStep = 2; updateTutorialBox(); } 
            else if (gameState.tutorialStep === 9) { gameState.tutorialStep = 10; updateTutorialBox(); }
        } else { showNotification("سنگ کافی نداری!", "warning"); }
    };
    const txtCancelUnlock = document.getElementById('txtCancelUnlock'); if (txtCancelUnlock) txtCancelUnlock.onclick = () => unlockModal.style.display = 'none';
    ...
    dispatchTroopsModal.innerHTML = `<h3 id="txtDispatchTitle" style="color: #f4d03f; margin-bottom: 5px;">اعزام نیرو</h3><p style="color: #aaa; font-size: 0.85rem; margin-bottom: 20px;"><span id="txtDispatchQ">اعزام شوند؟</span> <span id="modalRegionName" style="color:#f4d03f; font-weight:bold;"></span></p><div id="dispatchTroopsBtns" style="display: flex; flex-wrap: wrap; justify-content: center; gap: 10px; margin-bottom: 20px;"></div><button id="txtCancelDispatch" style="width: 100%; padding: 10px; background: transparent; border: 1px solid #8a7a6a; border-radius: 6px; color: #f5e6c8; cursor: pointer;">انصراف</button>`;
    if (gameScreen) gameScreen.appendChild(dispatchTroopsModal);
    const txtCancelDispatch = document.getElementById('txtCancelDispatch'); if (txtCancelDispatch) txtCancelDispatch.onclick = () => { dispatchTroopsModal.style.display = 'none'; dispatchTroopsModal.style.opacity = 0; };

    const panelExplore = document.createElement('div');
    panelExplore.className = 'floating-panel'; panelExplore.id = 'panelExplore';
    panelExplore.innerHTML = `<div class="panel-header" style="display:flex; justify-content:space-between; align-items:center; padding:12px 16px; border-bottom:1px solid rgba(201,168,76,0.1);"><h3 id="txtExploreTitle" style="font-size:1rem; color:#f4d03f;">اکتشاف</h3><button id="closeExplorePanel" style="background:none; border:none; color:#8a9aaa; font-size:1.2rem; cursor:pointer;">✕</button></div><div class="panel-body" style="padding:16px; max-height: 400px; overflow-y: auto;"><div class="build-item-new" style="display:flex; flex-direction:column; gap:8px; margin-bottom:15px; padding:12px;"><div style="font-size:1.1rem; font-weight:700; color:#f5e6c8;">کویر لوت</div><div class="region-risk" data-val="۰ تا ۱۰۰ درصد" style="font-size:0.8rem; color:#aaa;">مقدار تلفات: ۰ تا ۱۰۰ درصد</div><div class="region-reward" data-val="۳۰ چوب، ۳۰ سنگ، ۲۰ غذا، ۵ سوخت" style="font-size:0.8rem; color:#f4d03f;">پاداش هر بازگشته: ۳۰ چوب، ۳۰ سنگ، ۲۰ غذا، ۵ سوخت</div><button id="dispatchLut" class="build-btn dispatch-text" style="width:100%; margin-top:5px;">اعزام نیرو</button></div><div class="build-item-new" style="display:flex; flex-direction:column; gap:8px; margin-bottom:15px; padding:12px;"><div style="font-size:1.1rem; font-weight:700; color:#f5e6c8;">آبشار لاتون</div><div class="region-risk" data-val="۰ تا ۷۰ درصد" style="font-size:0.8rem; color:#aaa;">مقدار تلفات: ۰ تا ۷۰ درصد</div><div class="region-reward" data-val="۱۰ غذا، ۲۰ سنگ" style="font-size:0.8rem; color:#f4d03f;">پاداش هر بازگشته: ۱۰ غذا، ۲۰ سنگ</div><button id="dispatchLaton" class="build-btn dispatch-text" style="width:100%; margin-top:5px;">اعزام نیرو</button></div><div class="build-item-new" style="display:flex; flex-direction:column; gap:8px; padding:12px;"><div style="font-size:1.1rem; font-weight:700; color:#f5e6c8;">تخت جمشید</div><div class="region-risk" data-val="۰ تا ۴۰ درصد" style="font-size:0.8rem; color:#aaa;">مقدار تلفات: ۰ تا ۴۰ درصد</div><div class="region-reward" data-val="۱۰ چوب، ۱۰ سنگ" style="font-size:0.8rem; color:#f4d03f;">پاداش هر بازگشته: ۱۰ چوب، ۱۰ سنگ</div><button id="dispatchPersepolis" class="build-btn dispatch-text" style="width:100%; margin-top:5px;">اعزام نیرو</button></div></div>`;
    if (gameScreen) gameScreen.appendChild(panelExplore);

    const panelRequests = document.createElement('div');
    panelRequests.className = 'floating-panel'; panelRequests.id = 'panelRequests';
    panelRequests.innerHTML = `<div class="panel-header" style="display:flex; justify-content:space-between; align-items:center; padding:12px 16px; border-bottom:1px solid rgba(201,168,76,0.1);"><h3 style="font-size:1rem; color:#f4d03f;">درخواست‌ها</h3><button id="closeRequestsPanel" style="background:none; border:none; color:#8a9aaa; font-size:1.2rem; cursor:pointer;">✕</button></div><div class="panel-body" id="requestTrackerBody" style="padding:16px; max-height: 400px; overflow-y: auto;"><p class="no-req-msg" style="color: #aaa; text-align: center; font-size: 0.9rem;">در حال حاضر درخواستی وجود ندارد.</p></div>`;
    if (gameScreen) gameScreen.appendChild(panelRequests);
    const closeRequestsPanel = document.getElementById('closeRequestsPanel'); if (closeRequestsPanel) closeRequestsPanel.onclick = () => panelRequests.classList.remove('panel-open');

    const panelBuild = document.getElementById('panelBuild');
    if (panelBuild) panelBuild.innerHTML = `<div class="panel-header" style="display:flex; justify-content:space-between; align-items:center; padding:12px 16px; border-bottom:1px solid rgba(201,168,76,0.1);"><h3 id="txtBuildTitle" style="font-size:1rem; color:#f4d03f;">ساخت و ساز</h3><button id="closeBuildPanel" style="background:none; border:none; color:#8a9aaa; font-size:1.2rem; cursor:pointer;">✕</button></div><div class="panel-body" style="padding:16px;"><div class="build-item-new" style="display:flex; justify-content:space-between; align-items:center; background:rgba(255,255,255,0.05); border-radius:10px; padding:15px; border:1px solid rgba(255,255,255,0.1); margin-bottom: 15px;"><div class="build-info-text" style="display:flex; flex-direction:column; gap:5px;"><div id="txtBuildHouse" class="build-name" style="font-size:1.1rem; font-weight:700; color:#f5e6c8;">ساخت خانه جدید</div><div style="font-size:0.8rem; color:#aaa;">هزینه: ۱۰ چوب</div><div class="build-buttons" style="display:flex; gap:8px; margin-top:5px;"><button id="txtBuildBtn" class="build-btn" style="padding:8px 16px; background:linear-gradient(145deg, #f4d03f, #c9a84c); border:none; border-radius:6px; color:#1a1a2e; font-weight:700; cursor:pointer;">ساخت</button></div></div><img src="house.png?t=${new Date().getTime()}" style="width:80px; height:80px; object-fit:contain; border-radius:8px; background:#111; border:1px solid #333;"></div><div class="build-item-new" style="display:flex; justify-content:space-between; align-items:center; background:rgba(255,255,255,0.05); border-radius:10px; padding:15px; border:1px solid rgba(255,255,255,0.1);"><div class="build-info-text" style="display:flex; flex-direction:column; gap:5px;"><div class="build-name" style="font-size:1.1rem; font-weight:700; color:#f5e6c8;">ساخت نیروگاه</div><div style="font-size:0.8rem; color:#aaa;">هزینه: ۱۰ سنگ</div><div class="build-buttons" style="display:flex; gap:8px; margin-top:5px;"><button id="selectPowerPlant" class="build-btn" style="padding:8px 16px; background:linear-gradient(145deg, #f4d03f, #c9a84c); border:none; border-radius:6px; color:#1a1a2e; font-weight:700; cursor:pointer;">ساخت</button></div></div><img src="powerplant.png?t=${new Date().getTime()}" style="width:80px; height:80px; object-fit:contain; border-radius:8px; background:#111; border:1px solid #333;"></div></div>`;

    const panelMovePop = document.createElement('div');
    panelMovePop.className = 'floating-panel'; panelMovePop.id = 'panelMovePop';
    panelMovePop.innerHTML = `<div class="panel-header" style="display:flex; justify-content:space-between; align-items:center; padding:12px 16px; border-bottom:1px solid rgba(201,168,76,0.1);"><h3 id="txtMovePopTitle" style="font-size:1rem; color:#f4d03f;">انتقال جمعیت</h3><button id="closeMovePopPanel" style="background:none; border:none; color:#8a9aaa; font-size:1.2rem; cursor:pointer;">✕</button></div><div class="panel-body" style="padding:16px; text-align:center;"><p id="txtMovePrompt" style="color:#f5e6c8; margin-bottom:10px;">چند نفر منتقل شوند؟</p><div id="movePopBtns" style="display:flex; justify-content:center; gap:10px; flex-wrap:wrap; margin:15px 0;"></div><button id="txtMovePopBtn" class="build-btn" style="width:100%; margin-top:10px;">انتقال</button></div>`;
    if (gameScreen) gameScreen.appendChild(panelMovePop);
    ...
    if (gameScreen) gameScreen.appendChild(tracker);
    ...
    resultModal.innerHTML = `<h3 id="txtResultTitle" style="color: #f4d03f; text-align: center; margin-bottom: 15px;">گزارش اکتشاف</h3><div id="resultText" style="color: #e8dcc8; font-size: 1rem; line-height: 1.8;"></div><button id="txtCloseResult" style="margin-top: 15px; width: 100%; padding: 10px; background: linear-gradient(145deg, #f4d03f, #c9a84c); border: none; border-radius: 6px; color: #1a1a2e; font-weight: 700; cursor: pointer;">دریافت</button>`;
    if (gameScreen) gameScreen.appendChild(resultModal);
    const txtCloseResult = document.getElementById('txtCloseResult'); if (txtCloseResult) txtCloseResult.onclick = () => resultModal.style.display = 'none';
    ...
    function switchTab(tab) {
        const v = document.getElementById('contentVideo'); if(v) v.style.display = tab === 'video' ? 'block' : 'none';
        const a = document.getElementById('contentAudio'); if(a) a.style.display = tab === 'audio' ? 'block' : 'none';
        const c = document.getElementById('contentControls'); if(c) c.style.display = tab === 'controls' ? 'block' : 'none';
        const tv = document.getElementById('tabVideo'); if(tv) tv.style.color = tab === 'video' ? '#f4d03f' : '#aaa';
        const ta = document.getElementById('tabAudio'); if(ta) ta.style.color = tab === 'audio' ? '#f4d03f' : '#aaa';
        const tc = document.getElementById('tabControls'); if(tc) tc.style.color = tab === 'controls' ? '#f4d03f' : '#aaa';
    }
    ...
    if (volumeSlider) {
        ...
        volumeSlider.oninput = () => { bgMusic.volume = parseFloat(volumeSlider.value); const bm = document.getElementById('btnMute'); if(bm) bm.innerText = bgMusic.volume === 0 ? LANG[gameState.currentLang].unmute : LANG[gameState.currentLang].mute; };
    }
    const btnMute = document.getElementById('btnMute');
    if (btnMute) btnMute.onclick = () => {
        if (bgMusic.volume > 0) { bgMusic.volume = 0; if (volumeSlider) volumeSlider.value = 0; btnMute.innerText = LANG[gameState.currentLang].unmute; } 
        else { bgMusic.volume = 0.4; if (volumeSlider) volumeSlider.value = 0.4; btnMute.innerText = LANG[gameState.currentLang].mute; }
    };

    const langSelect = document.getElementById('langSelect');
    if (langSelect) langSelect.onchange = (e) => { applyLang(e.target.value); showNotification(LANG[gameState.currentLang].langChanged, "info"); };

    updateBindTexts();
    const bindUp = document.getElementById('bindUp'); if (bindUp) bindUp.onclick = () => { gameState.rebindingKey = 'up'; bindUp.innerText = '...'; };
    const bindDown = document.getElementById('bindDown'); if (bindDown) bindDown.onclick = () => { gameState.rebindingKey = 'down'; bindDown.innerText = '...'; };
    const bindLeft = document.getElementById('bindLeft'); if (bindLeft) bindLeft.onclick = () => { gameState.rebindingKey = 'left'; bindLeft.innerText = '...'; };
    const bindRight = document.getElementById('bindRight'); if (bindRight) bindRight.onclick = () => { gameState.rebindingKey = 'right'; bindRight.innerText = '...'; };
    ...
    if (txtBuildBtn) txtBuildBtn.onclick = () => {
        if (gameState.tutorialStep > 0 && gameState.tutorialStep !== 3) { showNotification("لطفاً طبق آموزش پیش برو!", "warning"); return; }
        gameState.isPlacing = true; gameState.placingType = 'house'; if (panelBuild) panelBuild.classList.remove('panel-open');
    ...
    if (selectPowerPlant) selectPowerPlant.onclick = () => {
        if (gameState.tutorialStep > 0 && gameState.tutorialStep !== 11) { showNotification("لطفاً طبق آموزش پیش برو!", "warning"); return; }
        gameState.isPlacing = true; gameState.placingType = 'powerplant'; if (panelBuild) panelBuild.classList.remove('panel-open');
    ...
    const openDispatchModal = (regionName) => {
        ...
        const mrn = document.getElementById('modalRegionName'); if(mrn) mrn.innerText = regionName;
        ...
        for(let i=1; i<=maxTroops; i++) {
            ...
            b.onclick = () => {
                ...
                dispatchTroopsModal.style.display = 'none'; dispatchTroopsModal.style.opacity = 0;
    ...
    const closeMovePopPanel = document.getElementById('closeMovePopPanel');
    if (closeMovePopPanel) closeMovePopPanel.onclick = () => { if (panelMovePop) panelMovePop.classList.remove('panel-open'); if (gameState.tutorialStep === 15) { gameState.tutorialStep = 14; updateTutorialBox(); } };
    const txtMovePopBtn = document.getElementById('txtMovePopBtn');
    if (txtMovePopBtn) txtMovePopBtn.onclick = () => {
        if (gameState.moveAmount > 0) {
            gameState.isMovingPop = true; if (panelMovePop) panelMovePop.classList.remove('panel-open');
            showNotification("حالا روی خونه مقصد کلیک کن", "info");
            if (gameState.tutorialStep === 15) { gameState.tutorialStep = 16; updateTutorialBox(); }
        }
    };

    const tutorialHTML = `<div id="tutorialBox" style="position: fixed; bottom: 90px; left: 50%; transform: translateX(-50%); width: 400px; max-width: 95%; background: rgba(10, 14, 26, 0.95); border: 1px solid rgba(201, 168, 76, 0.2); border-radius: 16px; z-index: 400; display: none; flex-direction: row; gap: 15px; padding: 20px; align-items: center; box-shadow: 0 0 20px rgba(0,0,0,0.5);"><div style="flex: 1; text-align: right;"><h3 style="color: #f4d03f; margin-bottom: 10px; font-size: 1rem;">آموزش بازی</h3><p id="tutorialText" style="color: #e8dcc8; font-size: 0.85rem; line-height: 1.6;"></p><button id="tutorialBtn" style="margin-top: 12px; padding: 8px 20px; background: linear-gradient(145deg, #f4d03f, #c9a84c); border: none; border-radius: 6px; color: #1a1a2e; font-weight: 700; cursor: pointer; display: none;">متوجه شدم</button></div><img src="guide.png?t=${new Date().getTime()}" style="width: 90px; height: 90px; object-fit: contain; border-radius: 10px; background: #111; border: 1px solid #333; flex-shrink: 0;"></div>`;
    if (gameScreen) gameScreen.insertAdjacentHTML('beforeend', tutorialHTML);
}
window.onload = initGame;                let saved = saveGame(true, exitAfter);
                if (saved && !exitAfter) showNotification("بازی با موفقیت ذخیره شد!", "success");
            } else { showNotification("لطفاً یک نام معتبر وارد کنید.", "warning"); }
        };
        document.getElementById('cancelSaveNameBtn').onclick = () => { modal.style.display = 'none'; };
...
function initGame() {
    ...
    // === ایمن‌سازی کامل دکمه‌های شروع و نصب ===
    const startBtnHero = document.getElementById('startBtnHero');
    if (startBtnHero) {
        startBtnHero.onclick = () => {
            gameState.currentSaveName = null; 
    ...
    if (heroActions && startBtnHero) {
        ...
        installBtn.id = 'btnInstallGame';
        installBtn.className = 'btn-primary';
        installBtn.innerText = 'نصب بازی روی دستگاه';
        installBtn.style.marginRight = '15px';
        installBtn.style.background = 'linear-gradient(135deg, #8e44ad, #9b59b6)';
        heroActions.appendChild(installBtn);

        window.addEventListener('beforeinstallprompt', (e) => {
            e.preventDefault();
            deferredPrompt = e;
            installBtn.innerText = 'نصب بازی';
            showNotification("می‌توانید بازی را نصب کنید!", "info");
        });
        ...
        installBtn.addEventListener('click', () => {
            if (deferredPrompt) {
                ...
                deferredPrompt.userChoice.then((choiceResult) => {
                    if (choiceResult.outcome === 'accepted') {
                        showNotification("بازی با موفقیت نصب شد!", "success");
                        installBtn.innerText = 'بازی نصب شد';
                ...
                const isIOS = /iPad|iPhone|iPod/.test(navigator.userAgent) && !window.MSStream;
                if (isIOS) {
                    showNotification("در آیفون: روی دکمه Share (آیکون مربع با فلش بالا) بزنید و Add to Home Screen را انتخاب کنید.", "info");
                } else {
                    showNotification("برای نصب، از منوی مرورگر (سه نقطه) گزینه Install App را انتخاب کنید.", "info");
                }
            }
